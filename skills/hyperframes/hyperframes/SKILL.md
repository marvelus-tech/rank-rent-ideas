...
## Quality Standard: YC / a16z Sizzle

Every video produced through this skill must meet the production quality of a heavily funded startup's sizzle reel — the kind shown at Y Combinator Demo Day or in an Andreessen Horowitz portfolio highlight. This is not "good for AI." This is genuinely good.

**What that means:**
- **Pacing:** No dead air. Every frame earns its place. Cuts are intentional, not accidental.
- **Motion:** Smooth, purposeful animation. No generic fade-ins. Elements arrive with energy and settle with precision.
- **Typography:** Bold, confident type at video scale (60px+ headlines, 20px+ body). No web UI opacity. Text is the hero.
- **Color:** Intentional palette, high contrast, no lazy defaults (#333, #3b82f6, Roboto are banned). Every hex is a deliberate choice.
- **Transitions:** 2-3 WebGL shader transitions max per video. The rest are hard cuts with entrance animations carrying the visual weight.
- **Audio:** Professional voiceover (Kokoro TTS: `af_sky`, `af_heart`, `am_michael` for marketing). Captions are kinetic and styled, not afterthoughts.
- **Narrative:** Hook → Story → Proof → CTA. Every video has a clear arc.

**The bar:** If this video were played before a Sequoia partner meeting, would it feel out of place? If yes, iterate.

## The Agent Skill Stack

HyperFrames ships 5 official skills. Install with:
...
Scaffold instantly with correct `data-*` attributes and pre-wired GSAP timelines:
...
|----------|------|----------|
| **`kinetic-type`** | Bold, dramatic typography animations | **Promos, intros, title cards** — the "exceptional promo" sweet spot |
...
## Catalog Blocks: Drop-in Production Components

Install high-value blocks via CLI:
...
- `cinematic-zoom` — dramatic zoom-through transition
...
**Block categories:** transitions, overlays, captions, charts, maps, VFX effects. Browse the full registry at [hyperframes.heygen.com/catalog](https://hyperframes.heygen.com/catalog).

## Shader Transitions: 14 Cinematic Options

WebGL pixel-shader transitions composited frame-by-frame via HyperShader.
...
## Seven-Step Production Pipeline (HeyGen's Internal Method)

Every production-grade launch video follows this artifact chain:
...
## Creative Use Cases & Community Inspiration
