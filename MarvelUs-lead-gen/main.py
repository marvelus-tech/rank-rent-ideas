...
from src.scoring import score_lead as legacy_score_lead
...
def _priority_categories(categories: list[str], full_service_bias: bool = False) -> list[str]:
    """
    Phase 3: Prioritize categories for full-service leads.
    When full_service_bias=True, strongly prefer categories that commonly produce
    ...
    # Phase 3 full-service priority list (curated for multi-opportunity potential)
    full_service_priorities = [
        "med spa", "beauty clinic", "skin clinic",
        "plumbers", "electricians", "hvac", "roofers",
        "dental clinic", "chiropractic", "physiotherapists",
        "lawyers", "accountants", "cleaning services",
    ...
    priority_list = full_service_priorities if full_service_bias else HIGH_VALUE_CATEGORIES
    ...
    # Append remaining categories in original order
    for idx, category in indexed:
...
def discover(config: dict, mode: str = "api", store: LeadStore | None = None, daily_limit: int = 10):
    ...
    if mode == "browser":
        ...
        # Phase 3: full-service bias from config
        full_service_bias = bool(config.get("discovery", {}).get("full_service_bias", False))
        prioritized_categories = _priority_categories(categories, full_service_bias=full_service_bias)
...
def analyze_and_score(df: pd.DataFrame, config: dict, enrich_contacts: bool = True):
    ...
    for _, row in df.iterrows():
        r = row.to_dict()
        # Phase 1: pass config so WebsiteAnalyzer respects seo.enabled + feature flag
        r.update(analyzer.analyze(r.get("website"), maps_phone=r.get("phone"), config=config))

        # Phase 2: Multi-dimensional scoring (full-service upgrade)
        if config.get("features", {}).get("enable_multi_dimensional_scoring", False):
            r.update(score_lead_multi(r, config))
        else:
            r.update(legacy_score_lead(r, config["scoring"]))

        # Phase 7: Auto-outreach draft generation (using Phase 1-5 intelligence)
        outreach_cfg = config.get("outreach", {})
        if outreach_cfg.get("enabled", False) or config.get("features", {}).get("enable_outreach_generation", True):
            try:
                r.update(generate_outreach_drafts(r, config))
            except Exception as exc:  # noqa: BLE001
                r["outreach_sms"] = ""
                r["outreach_email_subject"] = ""
                r["outreach_email_body"] = ""
                r["outreach_error"] = str(exc)
    ...
    # Phase 4: Ensure all Phase 1+2 fields are present for outputs (schema safety)
    for col in ["seo_health_score", "ai_voice_fit_score", "web_seo_fit_score",
                "reputation_fit_score", "booking_conversion_fit_score",
                "overall_help_score", "is_multi_opportunity", "recommended_primary_service"]:
        if col not in out.columns:
            out[col] = 0 if "score" in col else False if "multi" in col else ""
    return out
