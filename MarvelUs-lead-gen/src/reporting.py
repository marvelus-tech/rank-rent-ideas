...
def write_daily_report(all_df, new_df, stats: dict, stage_counts: dict[str, int], output_dir: str):
    ...
    lines += ["", "## Top New High-Priority Leads (Daily Delta)", "", "(Phase 1+2: SEO + Multi-dimensional scoring enabled)", ""]

    if high_priority is None or high_priority.empty:
        lines.append("- No new high-priority leads today")
    else:
        for _, row in high_priority.sort_values("overall_help_score", ascending=False).head(20).iterrows():
            email = row.get('email', '') or 'n/a'
            phone = row.get('phone', '') or 'n/a'
            seo = int(row.get('seo_health_score', 0) or 0)
            overall = int(row.get('overall_help_score', row.get('lead_score', 0)) or 0)
            multi = "YES" if row.get('is_multi_opportunity') else "No"
            rec = row.get('recommended_primary_service', 'AI Voice')
            missing = []
            if email == 'n/a':
            ...
            lines.append(
                ...
                f"Overall: {overall} | SEO Health: {seo} | Multi: {multi} | Rec: {rec} | "
